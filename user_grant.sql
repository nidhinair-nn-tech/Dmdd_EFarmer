BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            IF SQLCODE != -1918 THEN -- Ignore "user does not exist" error
                RAISE;
            END IF;
    END;
    ...
    -- Dropping user logistic_partner if it exists
    BEGIN
        EXECUTE IMMEDIATE 'DROP USER logistic_partner CASCADE';
    EXCEPTION
        WHEN OTHERS THEN
            IF SQLCODE != -1918 THEN -- Ignore "user does not exist" error
                RAISE;
    ...
    -- Grant specific privileges to logistic_partner
